...
<body scroll="no" id="info" onselectstart="return false;">
<!-- 显示返回主页面倒计时时间 -->
...
<script type="text/javascript">
            ...
            function () {
                ...
                $("#time").text(time);
                  if(time<4 && time>0){
                    ...
                    //跳转页面
                   // cancleOrder();
                	window.location.href="zzfkresulttemp.jsp";
                }
            }, 1000);
    }
    ...
    //微信充值下单
    function wxorder() {
    	$.session.set("zzfkstatus","0");
        $("#xming0").text(window.parent.xming0);
        $("#carNo").text(window.parent.cardNo);
       	$("#zfye").text(value);
        var datas = {
            "funcid": "M07.02.03.29",
            "cxdm00": cardNo,
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if (Data.retcode == "0") {
                    ...
                    $("#erweima").show();
                    $("#quxiao").show();
                    queryOrderTimeout = setTimeout("queryOrder()", 3000);//查询订单状态
                } else {
                    $("#tip_div").show();
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                var Data = eval(json);
               	...
               	//Data.retcode = "0"
            	//Data.retbody[0].czztbz="3";
               	
                if (Data.retcode == "0") {
                    czztbz = Data.retbody[0].czztbz;//充值状态标识
                    var cgjysj = Data.retbody[0].cgjysj;//交易时间
                    switch (czztbz) {
                        case "3":
                            ...
                            $("#quxiao").hide();
                            time = 90;
                            $.session.set("zzfkstatus","1");
                            $.session.set("cgjysj",cgjysj);//建档确认入参使用
                            goNext();//跳转
                            break;
                        case "5":
