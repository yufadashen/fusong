...
<div id="time" style="position:absolute;top:3px;left:900px;font-size:60px;color: #FF0000;"></div>
	<div id="titleArea">
		<font style="margin-left:40px;">选择挂号科室</font>
	</div>
	<div id="showArea">
		<div style="font-size:44px;margin-top:25px;"><span style="margin-left:-170px;">请选择要挂号的科室!</span></div>
...
<script type="text/javascript">
	...
	function clickKeshi(obj) {
		...
		var hisksbh00 = obj.children[2].innerText;
		$.session.set('keshi', keshi);
		$.session.set('ksbh00', ksbh00);
		$.session.set('hisksbh00', hisksbh00);

		window.location.href = "${pageContext.request.contextPath}/jsp/mzgh/ghwb.jsp";
		$("#waiting").show();
	...
	function getKeshi1() {
		...
		$.ajax({
			...
			success : function(json) {
				//$.getJSON("keshi.json",function(json){//测试数据
				var Data = eval(json);
				if (Data.retcode == "0") {
					...
					for (var i = 0; i < length; i = i + 3) {
						if (i + 2 < length) {
							$("#info_t").append(
									"<tr>"
									+"<td><div class=\"models\" onclick=\"clickKeshi(this)\"> <span class=\"ksWz\" style=\"color:white;\">" + Data.retbody[i].mzksmc + "</span>"
									+"<span class=\"ksbh\" style=\"color:white;\">" + Data.retbody[i].mzksbh + "</span>"
									+"<span class=\"ksbh\" style=\"color:white;\">" + Data.retbody[i].ksbh00 + "</span></div></td>"
									
									+"<td><div class=\"models\" onclick=\"clickKeshi(this)\"> <span class=\"ksWz\" style=\"color:white;\">" + Data.retbody[i+1].mzksmc + "</span>"
									+"<span class=\"ksbh\" style=\"color:white;\">" + Data.retbody[i+1].mzksbh + "</span>"
									+"<span class=\"ksbh\" style=\"color:white;\">" + Data.retbody[i+1].ksbh00 + "</span></div></td>"
									
									+"<td><div class=\"models\" onclick=\"clickKeshi(this)\"> <span class=\"ksWz\" style=\"color:white;\">" + Data.retbody[i+2].mzksmc + "</span>"
									+"<span class=\"ksbh\" style=\"color:white;\">" + Data.retbody[i+2].mzksbh + "</span>"
									+"<span class=\"ksbh\" style=\"color:white;\">" + Data.retbody[i+2].ksbh00 + "</span></div></td>"
									+"</tr>");
						} else if (i + 1 < length) {
							$("#info_t").append(
									"<tr>"
