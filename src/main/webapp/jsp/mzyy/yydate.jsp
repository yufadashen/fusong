...
<body scroll="no" onselectstart="return false">
	...
	<div id="titleArea">
		<font style="margin-left:40px;">选择预约日期</font>
	</div>
	<div id="showArea">
		<div style="font-size:44px;margin-top:25px;"><span style="margin-left:-170px;">请选择要预约的日期!</span></div>
...
<script type="text/javascript">
	...
	function clickKeshi(obj) {
		keshi = obj.innerText.substr(0,10);
		$.session.set('yydate', keshi);
		window.location.href = "yydoctor.jsp";		
		$("#waiting").show();
	}
	...
	function getKeshi1() {
		var datas = {
			"funcid" : "M07.02.04.05",
			"mzksbh" : $.session.get("ksbh00")//科室编号
		 ...
		 */	
			...
			$.ajax({
				...
				success : function(json) {
					$("#waiting").hide();
					//$.getJSON("../mzgh/yydate.json",function(json){//测试数据
					var Data = eval(json);
					if (Data.retcode == "0") {
						...
						for (var i = 0; i < length; i = i + 3) {
							if (i + 2 < length) {
								$("#info_t").append(
										"<tr>"
										+"<td><div class=\"models\" onclick=\"clickKeshi(this)\"> <span class=\"ksWz\">" + Data.retbody[i].yyrq00 + "<br>" 
										+ Data.retbody[i].week00 + "</span></div></td>"
										
										+"<td><div class=\"models\" onclick=\"clickKeshi(this)\"> <span class=\"ksWz\">" + Data.retbody[i+1].yyrq00 + "<br>" 
										+ Data.retbody[i+1].week00 + "</span></div></td>"
										
										+"<td><div class=\"models\" onclick=\"clickKeshi(this)\"> <span class=\"ksWz\">" + Data.retbody[i+2].yyrq00 + "<br>" 
										+ Data.retbody[i+2].week00 + "</span></div></td>"
										+"</tr>");
							} else if (i + 1 < length) {
								$("#info_t").append(
										"<tr>"
